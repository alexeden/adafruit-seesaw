...
pub trait EncoderModule<D: crate::Driver>: GpioModule<D> {
    ...
    fn enable_button(&mut self) -> Result<(), crate::SeesawError<D::Error>> {
        self.set_pin_mode(Self::ENCODER_BTN_PIN, PinMode::InputPullup)
            .map(|_| self.driver().delay_us(125))
    }

    fn button(&mut self) -> Result<bool, crate::SeesawError<D::Error>> {
        self.digital_read(Self::ENCODER_BTN_PIN)
    }

    fn delta(&mut self) -> Result<i32, crate::SeesawError<D::Error>> {
        let addr = self.addr();
        self.driver()
            .read_i32(addr, DELTA)
    ...
    fn disable_interrupt(&mut self) -> Result<(), crate::SeesawError<D::Error>> {
        let addr = self.addr();
        self.driver()
    ...
    fn enable_interrupt(&mut self) -> Result<(), crate::SeesawError<D::Error>> {
        let addr = self.addr();
        self.driver()
    ...
    fn position(&mut self) -> Result<i32, crate::SeesawError<D::Error>> {
        let addr = self.addr();
        self.driver()
    ...
    fn set_position(&mut self, pos: i32) -> Result<(), crate::SeesawError<D::Error>> {
        let addr = self.addr();
        self.driver()
